...
\begin{enumerate}
	\item $f\brak{x} = \mydet {
		...
		1 & \cos^2 x & \cos^2 x}$\\
		Then $\int_0^{\frac{\pi}{2}}$ $f\brak	{x}dx$ = .......
\hfill{(1987 - 2 Marks)}\\ 
...
\item Let $\frac {d}{dx}F\brak{x}=\frac {e^{\sin x}}{x},x>0$.If $\int_1^4 \frac{2e^{\sin x^2}}{x}=F\brak{k}-F\brak{1}$ then one of the possible valued of$ k$ is ...
\hfill{(1997 - 2 Marks)}\\
\end{enumerate}
